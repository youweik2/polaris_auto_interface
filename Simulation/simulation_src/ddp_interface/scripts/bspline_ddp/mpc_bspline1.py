...
obstacles = [
    (0.0, 15, 1),#  (1, 20, 1), (-1, 25, 1) # , (-2.0, 25, 1.2), (1.0, 28, 1.0)
]
...
def Phi(x, x_goal):
    Qf = np.diag([5, 5, 5.0, 0.0, 0.1])
    return (x - x_goal).T @ Qf @ (x - x_goal)

def L(x, u, x_goal):
    error = x - x_goal
    Q = ca.diag(ca.DM([1.0, 1.0, 0.0, 0.0, 1.0]))  
    ...
    return 1 * state_cost + 10 * control_cost + obstacle_cost + alignment + 1 * boundary_cost + 10* phi_boundary_cost + 0.1 * v_boundary_cost
...
def main():
    ...
    x_current = ca.DM([-0.0, 3, np.pi/2, 0, 0])
