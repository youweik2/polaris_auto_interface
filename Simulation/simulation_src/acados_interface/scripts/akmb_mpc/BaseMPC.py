...
class GemCarOptimizer(object):
    ...
    def __init__(self, m_model, m_constraint, t_horizon, dt, obstacles, target):
        ...
        # Cost settings ***
        Q = np.diag([3.0, 5.0, 15.0, 5.0])  # States
        R = np.array([[100.0, 0.0],[0.0, 200.0]])            # Controls
    ...
    def plot_results(self, x_log, y_log, theta_log, v_log, a_log, steer_log):
        ...
        axs[2].plot(timeline, y_log, 'r-', label='y move')
                ...
                try:
                    ...
                    if (x_0 - self.target_x) ** 2 + (y_0 - self.target_y) ** 2 < 1:
                        ...
                        if self.plot_figures == True:
                            self.plot_results(x_log, y_log, theta_log, v_log, a_log, o_log)
